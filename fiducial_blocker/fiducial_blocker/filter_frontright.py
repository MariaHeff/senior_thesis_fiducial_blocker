...
import numpy as np
import cv2
class FiducialFilterNodeFrontRight(Node):
	def __init__(self):
		super().__init__('fiducial_filter_node_frontright')
		# Camera topic to filter
		self.camera_topic = "/spot1/camera/frontright/compressed"
		self.filtered_camera_topic = "/spot1/camera/frontright/filtered"
	...
	def camera_callback(self, msg):
		...
		camera_name = "frontright" # Adjust according to world objects format
...
def main(args=None):
	...
	node = FiducialFilterNodeFrontRight()
	rclpy.spin(node)
	node.destroy_node()
	rclpy.shutdown()
