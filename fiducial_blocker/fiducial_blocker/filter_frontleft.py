...
import numpy as np
import cv2
class FiducialFilterNodeFrontLeft(Node):
	def __init__(self):
		super().__init__('fiducial_filter_node_frontleft')
		# Camera topic to filter
		self.camera_topic = "/spot1/camera/frontleft/compressed"
		self.filtered_camera_topic = "/spot1/camera/frontleft/filtered"
	...
	def camera_callback(self, msg):
		...
		camera_name = "frontleft" # Adjust according to world objects format
...
def main(args=None):
	...
	node = FiducialFilterNodeFrontLeft()
	rclpy.spin(node)
	node.destroy_node()
	rclpy.shutdown()
